...
def index(request: Request, region: str = '', distance: str = '', pref: str = ''):
    ...
    query = '''
        ...
        AND e.name NOT LIKE '%試走会%'
        AND e.name NOT LIKE '%セミナー%'
        AND e.name NOT LIKE '%RUN CLUB%'
        AND e.name NOT LIKE '%ランクラブ%'
        AND e.name NOT LIKE '%変化走%'
        AND e.name NOT LIKE '%登拝%'
        AND e.name NOT LIKE '%枕投げ%'
        AND e.name NOT LIKE '%送迎バス%'
        AND e.name NOT LIKE '%ナイトクラス%'
        AND e.name NOT LIKE '%ビギナークラス%'
        AND e.name NOT LIKE '%マラソンキャンプ%'
        AND e.name NOT LIKE '%スイム&ラン%'
        AND e.name NOT LIKE '%ピクニックラン%'
        AND e.name NOT LIKE '%ジャーニー%'
